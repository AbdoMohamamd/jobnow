      ...
      <div className="flex h-full bg-secondary-grey  items-center justify-center  md:justify-start  md:w-6/10  md:box-border  lg:gap-12 xl:gap-16">
        ...
        >
          <Image
            src={Logo}
            loading={"eager"}
            className="w-28 md:h-28 lg:w-max lg:h-max "
          />
        </Link>
        ...
        <div
          className={`${
            isOpen ? "fixed" : "hidden"
          }  flex flex-col gap-y-5  bg-secondary-grey mt-20  top-0 left-0 right-0 bottom-0 z-20   md:mt-0  md:static md:flex md:flex-row`}
        >
          {" "}
          <button
            className={`${
          ...
          <Link
            href="#hero-section"
            className="h-10 text-center  text-xl font-poppins font-bold transition-all hover:border-b-4 box-border  hover:border-primary-color   px-4   ease-in-out duration-300 md:text-xs md:py-0 md:px-0  md:mx-4 md:h-5 lg:text-sm xl:text-base xl:mx-6 xl:h-6 "
          >
            Home
          </Link>
          <Link
